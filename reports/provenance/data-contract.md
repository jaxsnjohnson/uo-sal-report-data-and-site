...
## Consolidated browsing histories

`js/history.js` consolidates observations by exact name, date, and selected pay
measure without changing source-profile identities or generated JSON. It also
builds a coverage calendar from catalog dates. Missing classification counterparts
and interior annual-series years become null chart points; unknown FY measures
stay unknown. Missing reports are not inserted into the archive as invented PDFs.

The default search uses the latest available `annual_rate` date independently for
each classification. The `latest` period is computed within the selected report
series and measure before text, role, pay, FTE, or flag predicates. Earlier periods
remain searchable; deep links open all measures/periods for the linked name.

Personal charts keep every usable job amount as a point. A date with several
entries has no single line value; missing or unusable entries also break the line.
No appointment identity or combined salary is inferred. Calendar-scaled SVGs,
keyboard-focusable points, missing-data overlays, and source tables share these
same observations. The overlay toggle never changes the values or bridges gaps.
